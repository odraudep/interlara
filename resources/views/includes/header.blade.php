...
<header id="main-navbar" class="bg-gray-300">
    <nav class="container flex items-center">
        <a href="/" class="outline-none focus:opacity-60">
        ...
        <ul class="header__menu">
            @foreach ($menu_map as $item)
                <li><a href="{{ $item['path'] }}"
                        @if (Route::current()->getName() == $item['name']) aria-current="page" @endif>{{ $item['label'] }}</a>
                </li>
            @endforeach
        </ul>
        ...
        <ul class="flex items-center gap-6">
            ...
            <ul class="flex gap-4">
                <li>
                    <a href='/login'
                        class="text-black outline-none hover:opacity-80 focus:underline focus:opacity-80">Log In</a>
                </li>
                <li>
                    <a href='/signup'
                        class="text-black outline-none hover:opacity-80 focus:underline focus:opacity-80">Sign Up</a>
                </li>
            </ul>
        </ul>
    </nav>
